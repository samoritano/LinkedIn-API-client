...
class LinkedInTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testApi()
    {
        ...
        $generator = $this->getMock('Bokokode\LinkedIn\Http\UrlGenerator', ['getUrl']);
        ...
        $requestManager = $this->getMock('Bokokode\LinkedIn\Http\RequestManager', ['sendRequest']);
        ...
        $linkedIn = $this->getMock('Bokokode\LinkedIn\LinkedIn', ['getAccessToken', 'getUrlGenerator', 'getRequestManager'], [self::APP_ID, self::APP_SECRET]);
    ...
    {
        $linkedIn = $this->getMock('Bokokode\LinkedIn\LinkedIn', ['getAccessToken'], [self::APP_ID, self::APP_SECRET]);
        $linkedIn->expects($this->once())->method('getAccessToken')->willReturn(null);
        $this->assertFalse($linkedIn->isAuthenticated());

        $linkedIn = $this->getMock('Bokokode\LinkedIn\LinkedIn', ['api', 'getAccessToken'], [self::APP_ID, self::APP_SECRET]);
        $linkedIn->expects($this->once())->method('getAccessToken')->willReturn('token');
    ...
    public function testAccessTokenAccessors()
    {
        ...
        $auth = $this->getMock('Bokokode\LinkedIn\Authenticator', ['fetchNewAccessToken'], [], '', false);
        $auth->expects($this->once())->method('fetchNewAccessToken')->will($this->returnValue($token));

        $linkedIn = $this->getMock('Bokokode\LinkedIn\LinkedIn', ['getAuthenticator'], [], '', false);
    ...
    {
        $get = new \ReflectionMethod('Bokokode\LinkedIn\LinkedIn', 'getUrlGenerator');
        $get->setAccessible(true);
        $linkedIn = new LinkedIn(self::APP_ID, self::APP_SECRET);

        // test default
        $this->assertInstanceOf('Bokokode\LinkedIn\Http\UrlGenerator', $get->invoke($linkedIn));

        $object = $this->getMock('Bokokode\LinkedIn\Http\UrlGenerator');
    ...
    {
        $get = new \ReflectionMethod('Bokokode\LinkedIn\LinkedIn', 'getFormat');
        $get->setAccessible(true);
        $linkedIn = new LinkedIn(self::APP_ID, self::APP_SECRET);
    ...
    public function testLoginUrl()
    {
        ...
        $generator = $this->getMock('Bokokode\LinkedIn\Http\UrlGenerator', ['getCurrentUrl']);
        $generator->expects($this->once())->method('getCurrentUrl')->willReturn($currentUrl);

        $auth = $this->getMock('Bokokode\LinkedIn\Authenticator', ['getLoginUrl'], [], '', false);
        $auth->expects($this->once())->method('getLoginUrl')
            ->with($generator, ['redirect_uri' => $currentUrl])
            ->will($this->returnValue($loginUrl));

        $linkedIn = $this->getMock('Bokokode\LinkedIn\LinkedIn', ['getAuthenticator', 'getUrlGenerator'], [], '', false);
        $linkedIn->expects($this->once())->method('getAuthenticator')->willReturn($auth);
        $linkedIn->expects($this->once())->method('getUrlGenerator')->willReturn($generator);
    ...
    public function testLoginUrlWithParameter()
    {
        ...
        $generator = $this->getMock('Bokokode\LinkedIn\Http\UrlGenerator');
